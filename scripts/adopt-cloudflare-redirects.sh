...
for command in curl jq tofu; do
...
case "$ruleset_status" in
    ...
    200)
        ...
        if [[ "$rule_count" != "0" ]]; then
            echo "Refusing to replace an unmanaged shared Cloudflare redirect ruleset." >&2
            echo "Ruleset ID: ${ruleset_id}; existing rules: ${rule_count}." >&2
            echo "Model and import every existing redirect rule before enabling WWMTF management." >&2
        ...
        echo "Adopted empty ${PHASE} entry-point ruleset ${ruleset_id}"
        ;;
    *)
        jq -r '.errors[]?.message' "$ruleset_file" >&2
