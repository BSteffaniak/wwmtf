...
The container's internal nginx proxy exposes port 8080, validates host and same-origin requests, limits account POSTs, applies a 64 KiB body limit, strips permissive CORS headers, and adds CSP/privacy/security headers. It forwards streaming traffic to the renderer-neutral application on `127.0.0.1:8343`. Every request receives an `X-Request-ID`; nginx access logs include that ID, upstream status and timing, and the secret-free shared-state diagnostic classification when present. The application logs its release and Fly Machine identity at startup. Secret-safe aggregate counters are available from `/metrics`; transport logs and metrics must never include session, CSRF, invitation, rack, bag, or canonical-event values.
